...
**Pablo — project direction, Bayesian backend, and product/UI.**  
Pablo helped shape Grab the Crab from its earliest stages, driving the overall product direction and keeping the project centered on a clear ecological decision problem rather than an AI-first demo. He worked on the technical backend behind the inference loop — including the Bayesian belief model, probabilistic treatment of imperfect detections and effort, simulator/state integration, and the interfaces that turn field observations into an updated decision state. He also led the product-facing integration and UI refinement, translating the underlying mathematics into an interactive mission-control experience where judges can see evidence change belief and belief change the next mission.
