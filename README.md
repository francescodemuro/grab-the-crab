...
Pablo originated and developed the core project direction from the early concept stage, helping steer Grab the Crab through its major pivots: away from a generic invasive-species map and toward the post-detection decision problem that became the project's north star — *where should we search next, how much effort should we spend, and how should the mission change when new evidence arrives?* He remained focused on the end-to-end coherence of the system as the simulator, inference engine, planners and product evolved in parallel.

On the technical side, Pablo worked on the mathematical and probabilistic backbone of the project. He developed the Bayesian belief layer that represents multiple possible invasion extents together with uncertain detectability, and made field evidence explicitly effort-aware through the likelihood `P(no detection | occupied, effort=e) = (1-q)^e`. That made a weak non-detection and a strong non-detection mathematically different observations instead of treating both as “absence.” The backend exposes posterior occupancy probabilities, uncertainty and detectability belief to the planner while keeping the realized hidden world structurally separate, so the decision layer acts only on what could actually be known in the field. He also worked on connecting that inference state to the simulator, real monitoring context and planner-facing graph representation, turning raw detections/non-detections into the updated decision state that drives every replan.

A tangible outcome of that work is the causal loop at the center of the product: the same field result can change the posterior differently depending on survey effort, that posterior propagates across plausible spatial extents, and the next recommended site and effort can change before the hidden truth is ever revealed. Pablo also led the product-facing integration and much of the UI refinement, translating the mathematical state into an interactive mission-control experience where judges can inspect belief, uncertainty, probable worlds and resource use — and visibly watch **field evidence → belief change → mission change**.
